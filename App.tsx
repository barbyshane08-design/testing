...
const App = () => {
  ...
  if (screen === 'title') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center space-y-6 relative">
        <div className="w-32 h-32 rounded-3xl bg-[#111] border-2 border-primary flex items-center justify-center text-6xl shadow-[0_0_30px_rgba(64,196,255,0.3)]">
          🎴
        </div>
        <div>
           <h1 className="text-4xl font-black text-white tracking-tight">FLIPMASTER 7</h1>
           <p className="text-primary font-bold uppercase tracking-widest text-sm mt-1">Vengeance Edition</p>
        </div>
        ...
        <div className="w-full max-w-xs space-y-3 pt-6 z-10">
          ...
          <button onClick={handleNewGameRequest} className="w-full p-4 bg-white/10 hover:bg-white/20 border border-white/10 text-white font-black rounded-xl uppercase transition-all">
             New Game
          </button>
    ...
    <div className="min-h-screen flex flex-col p-4 max-w-lg mx-auto">
      ...
      {winner && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md">
            <div className="text-center p-8 animate-bounce-in">
                ...
                <button onClick={() => setScreen('title')} className="px-8 py-4 bg-primary text-black font-black rounded-xl">RETURN TO TITLE</button>
            </div>
        </div>
      )}
      ...
      <div className="flex-1 space-y-3 overflow-y-auto pb-32">
                ...
                <div className="flex justify-between items-center mb-3">
                   ...
                   <div className="flex flex-col gap-2 items-end">
                      ...
                      <button 
                         ...
                         disabled={p.busted}
                         className="flex items-center gap-2 bg-black/40 px-3 py-2 rounded-xl border border-white/20 min-w-[80px] justify-center"
                      >
                         {p.currentInput ? (
        ...
        <div className="flex gap-2 mt-4">
           ...
           <button onClick={(e) => {
               ...
               input.value = '';
           }} className="bg-white/10 px-4 rounded-xl font-bold text-2xl text-primary">+</button>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-bg via-bg to-transparent">
         <div className="max-w-lg mx-auto">
            <button onClick={submitRound} className="w-full bg-primary text-black font-black text-xl py-4 rounded-2xl shadow-lg shadow-primary/20 active:scale-95 transition-transform">
               SUBMIT ROUND
            </button>
         </div>
      </div>
      ...
      {/* Calculator Modal */}
      {calcTarget !== null && (
          <div className="fixed inset-0 z-40 bg-bg flex flex-col pt-safe">
              <Calculator 
                 mode={gameMode} 
